...
namespace M3DS {
    ...
    public:
        ...
        template <TrivialIOType T> [[nodiscard]] bool write(std::span<T> to) const noexcept;
...
namespace M3DS {
    ...
    template <TrivialIOType T>
    bool BinaryFile::write(std::span<T> to) const noexcept {
    ...
    template <TrivialIOType T>
    bool BinaryOutFile::write(std::span<T> to) const noexcept {
        return write(std::span<const T>{to});
    }
    ...
    template <TrivialIOType T>
    bool BinaryFileAccessor::write(std::span<T> to) const noexcept {
        return write(std::span<const T>{to});
    }
    ...
    template <TrivialIOType T>
    bool BinaryOutFileAccessor::write(std::span<T> to) const noexcept {
        return write(std::span<const T>{to});
    }
