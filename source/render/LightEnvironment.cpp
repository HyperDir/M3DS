...
static constexpr float toon_diffuse(const float x, [[maybe_unused]] float arg) {
    static constexpr float numDiffuseTones = 3;
...
namespace M3DS {
    LightEnv::LightEnv(const bool toonShaded) noexcept : mToonShaded(toonShaded) {
        C3D_LightEnvInit(&mLightEnv);

        if (mToonShaded) {
            static C3D_LightLut lut_Diffuse = LightLut_FromFunc(toon_diffuse, 0);
            static C3D_LightLut lut_Spec = LightLut_FromFunc(toon_specular, 5);

            C3D_LightEnvLut(get(), GPU_LUT_D0, GPU_LUTINPUT_LN, false, &lut_Diffuse);
            C3D_LightEnvLut(get(), GPU_LUT_D1, GPU_LUTINPUT_NH, false, &lut_Spec);
        } else {
    ...
    void LightEnv::bind() noexcept {
        C3D_LightEnvBind(&mLightEnv);
    }

    C3D_LightEnv* LightEnv::get() noexcept {
        return &mLightEnv;
    }

    const C3D_LightEnv* LightEnv::get() const noexcept {
        return &mLightEnv;
    }

    bool LightEnv::isToonShaded() const noexcept {
        return mToonShaded;
    }

    void LightEnv::material(const C3D_Material& material) {
